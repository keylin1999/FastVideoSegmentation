...
# slight modified code from video_seg.py to find out the first steady frame.
...
def video_runner(args, model, IMG_SIZE, MARGIN, patch_splitting):
    ...
    for vid_idx, video in enumerate(videos):
        # if video != 'CRA9': # CVAI-0763_LCX_RAO10_CAU35
        #     continue
        ...
        for index, image in enumerate(images):
            ...
            img = prob_map.cpu().detach().numpy()
            img = img.astype(np.uint8)
            img = img * 255
            normal_metric.update_np(img, label)
            metricss.append(copy.deepcopy(normal_metric))
        ...
        for i in range(len(images)):
            ...
            for index, image in enumerate(images):
                ...
                img = img.astype(np.uint8)
                img = img * 255
                video_metric.update_np(img, label)
            dic[video].append((normal_metric, video_metric))
            print(video_metric.f1_score(), normal_metric.f1_score())
            if normal_metric.f1_score() - video_metric.f1_score() < 0.001:
                break
...
def run():
    ...
    if output_path is None:
        ...
        output_path = f'output/{args.model}/{args.method}_{args.epoch}/{comment}{args.size}_{args.margin}_{args.label_thresh}_{args.area_thresh}_{args.cont}_{args.video_thresh}_{args.pixel_thresh}'
    print(output_path)
